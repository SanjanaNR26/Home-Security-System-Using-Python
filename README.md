...
**Usage**
Ensure the script file has a .py extension (rename a to a.py if needed).
Run from the folder containing the script:
A window titled "Security Feed" will appear showing the camera frames and bounding boxes for motion.
Press q in the window to quit.
The script prints logs to the console when it attempts to send email alerts.
